      ...
      >
        <div class="py-1">
          <template v-if="account">
            ...
            <button @click="$emit('wecom-alert', account); $emit('close')" class="flex w-full items-center gap-2 px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-dark-700">
              <Icon name="bell" size="sm" class="text-green-600" />
              {{ t('admin.wecomUsageAlert.menu') }}
            </button>
...
const emit = defineEmits(['close', 'test', 'stats', 'schedule', 'wecom-alert', 'duplicate', 'reauth', 'refresh-token', 'recover-state', 'reset-quota', 'set-privacy', 'create-spark-shadow'])
const { t } = useI18n()
